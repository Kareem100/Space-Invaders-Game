...
using namespace sf;
#define pos 500
bool is = false, upL = false, upR = false, downL = false, downR = true;
...
int counter_big_enemy_egg = 0, wait_after_asteroid = 0, counter_big_enemy_life = 0, bullet_counter = 0, ani = 0, button = 0, d_move;
...
void mohmed::mohmed_yehia(){
	...
	while (app.isOpen())
	{
		...
		for (auto b : entities)
		{
			...
			if ((a->name == "player" && b->name == "chicken_enenmy") || (a->name == "player" && b->name == "chicken_egg"))
			if (isCollide(a, b))
			{
				if (after_die == true)
					b->life = false;
		...
		cont++;
		for (auto c : entities){
			if (c->name == "chicken_enenmy"&&rand() % 150 == 0 && cont % 15 == 0){
